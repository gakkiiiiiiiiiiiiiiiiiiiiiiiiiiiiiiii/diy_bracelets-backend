...
export class BraceletCodeController {
  ...
  @Post('encode') encode(@Body() dto: EncodeBraceletCodeDto) { return { code: this.service.encode({ v: 1, ...dto }) }; }
  @Post('resolve') resolve(@Body() dto: ResolveBraceletCodeDto) { return this.service.resolve(dto.code); }
}
